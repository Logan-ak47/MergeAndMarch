...
## PROTOTYPE STATUS SNAPSHOT (April 2026)

- Phase 1 prototype now includes a playable combat foundation, not just grid + merge.
- Current implemented battle slice: portrait 4x2 grid, center-column starting lineup, drag-to-merge with tactical slow-mo, enemy spawning from above, downward movement, lane-based engagement, troop auto-attacks, and fail state on enemy escape.
- Current readability level: solid enough for prototype evaluation, with lane guides and simple attack motion now helping a lot, but it still needs projectiles, death FX, and stronger wave/state communication.
- Current control status: drag responsiveness improved after collider sizing fixes, but this should still be validated on a real mobile touch screen before being considered final.
- Immediate prototype priority is still readability/polish of the core combat loop before building deployment/cards/meta systems on top of it. The latest readability pass added lane guides, clearer enemy lane alignment, enemy attack pulses, and simple Knight/Archer attack motion.
...
3. Enemy waves march **downward from the top** of the screen toward your troops
...
### Layout: 4 Columns × 2 Rows (8 slots) — Vertical Orientation

```
        ↓ ↓ ↓ ENEMIES MARCH DOWN FROM TOP ↓ ↓ ↓

        [  ~65% of screen: BATTLEFIELD  ]

        Col 1      Col 2      Col 3      Col 4
Row 0:  [  Slot  ]  [  Slot  ]  [  Slot  ]  [  Slot  ]  ← FRONTLINE (top row)
Row 1:  [  Slot  ]  [  Slot  ]  [  Slot  ]  [  Slot  ]  ← BACKLINE (bottom row)

        [  Grid sits in bottom ~25% of screen  ]
```

- **Portrait orientation (9:16)** — matches Archero/Survivor.io layout
- Row 0 (top) = frontline (closest to enemies). Row 1 (bottom) = backline (safer)
- Columns are lateral positioning; rows determine range from enemies
- Grid centered horizontally, anchored to bottom of screen
- Enemies spawn above camera top edge, march downward
- If an enemy passes below the grid (below Row 1), the run fails
- Thumb-friendly: grid is exactly where thumbs rest in portrait mode
- Troops in the same column are "adjacent" for Healer purposes

### Starting Slots (Progression-Based)

- New players start with **4 of 8 slots filled** (center 2 columns, both rows)
...
|------|------|---------|-------------|-------------|-------|---------|
| Knight | Tank | 100 | 15 | 2.0s | Melee (own row + 1 row above) | Blocks enemies in front row |
| Archer | DPS | 40 | 12 | 0.8s | Infinite | Attacks from back row |
| Mage | AoE | 35 | 25 | 2.5s | Infinite (hits all enemies in a horizontal band) | Area damage |
...
- "Nearest" = shortest Y distance (vertical, for ranged: any enemy on field)
- Ties broken by leftmost column
...
### Range by Type
- **Knight:** Melee — can hit enemies within own row's Y zone + 1 cell height above (row 0 frontline)
- **Archer:** Any enemy on the field (infinite range, nearest by Y distance)
- **Mage:** All enemies in a horizontal band (same Y zone as nearest enemy — like a row-wide blast)
- **Healer:** Adjacent troops (same column ± 1 row, or same row ± 1 column)
- **Bomber:** Enemy that reaches bomber's row triggers explosion
...
- Enemies spawn **above the top edge** of the screen, march **downward**
- Spawn X position: randomized across the grid's column range for visual variety
- When an enemy reaches a row with a troop, it stops and attacks that troop
- If the troop dies, enemy resumes marching downward
- **If any enemy passes below the grid (below Row 1), the run ends**
...
|-------|----------|---------|
| Grunt | Marches down, basic attack | Any troop |
| Rusher | Fast downward movement, low HP | Bomber or AoE |
| Tank | Slow, high HP, high damage | Focused DPS (merged archers) |
| Flyer | Skips front row (row 0), attacks back row (row 1) directly | Mage or repositioned archers |
...
### Boss (Wave 16)
First boss: High HP + AoE slam that damages the entire front row (row 0) every 5 seconds. Future zones introduce boss variants with different mechanics (shield phases, summon adds, enrage timers).
...
### Screen 3: Battle
- Grid centered at bottom, enemies march downward from top
...
**Current prototype reality:** Grid/merge/basic combat is playable, lane guides and basic attack motion now make the battle much easier to parse, but the deployment system is not implemented yet and combat feedback is still placeholder-level. The next practical step is to keep improving readability and trust in the battle loop, not to expand scope immediately.
